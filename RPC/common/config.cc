...
namespace RPC{
	...
	RPC::Config::Config(const char* xmlfile){
		...
		READ_STRING_FROM_XML_NODE(port,server_node);
		READ_STRING_FROM_XML_NODE(io_threads,server_node);
    ...
    m_log_level.c_str(), m_log_file_name.c_str(), m_log_file_path.c_str(), m_log_file_max_size, m_log_sync_interval);
